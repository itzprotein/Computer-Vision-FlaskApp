pipeline {
    ...
    stages {
        stage('Git Checkout') {
            steps {
                checkout scmGit(branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/itzprotein/Computer-Vision-FlaskApp.git']])
            }
        }
        stage("Docker Build") {
            steps {
